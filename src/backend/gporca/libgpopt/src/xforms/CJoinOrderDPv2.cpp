	...
	)
{
	...
	for (ULONG ul=0; ul < group_info->m_best_expr_info_array->Size(); ul++)
	{
		...
		{
			if (gpos::ulong_max == best_ix || expr_info->TotalCost() < best_cost)
			{
				// we found a candidate with the best cost so far that satisfies the properties
				best_ix = ul;
				best_cost = expr_info->TotalCost();
			}
		}
	}
	...
	)
{
	...
	CDouble new_cost = new_expr_info->TotalCost();
	...
	for (ULONG ul=0; ul < group_info->m_best_expr_info_array->Size(); ul++)
	{
		...
		CDouble old_cost = expr_info->TotalCost();
		CDouble new_cost = new_expr_info->TotalCost();
	...
	)
{
	...
	for (; right_ix<right_size; right_ix++)
	{
		...
		if (NULL != join_expr_info)
		{
			...
			CDouble join_cost = join_expr_info->TotalCost();
	...
	for (ULONG lev=1; lev<num_levels; lev++)
	{
		...
		for (ULONG c=0; c<num_bitsets_this_level; c++)
		{
			...
			for (ULONG x=0; x<num_exprs; x++)
			{
				...
				os << "   Cost with Penalty: ";
				expr_info->TotalCost().OsPrint(os);
				os << std::endl;
				if (lev == 1)
				{
					os << "   Atom: " << std::endl;
